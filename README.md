...
스캔한 방(스플랫) 위에 2주차에 만든 안마의자 메시(GLB)를 얹어 한 장면으로 구성했습니다.

| | 파일 |
|---|---|
| 배경 (3DGS) | `assets/room.spz` |
| 오브젝트 (Mesh) | `assets/massage_chair_model.glb` |
| 뷰어 | `splat-twin.html` |


메시와 실제 안마의자가 겹치지 않도록 supersplat에서 안마의자 영역을 삭제하고 그 자리에 메시를 배치했습니다.

| | 스플랫 개수 | 크기 |
|---|---|---|
| 삭제 전 | 346,372 | 7.14 MB |
| 삭제 후 | **294,674** | **6.09 MB** |

삭제된 영역(0.70 × 1.00 × 0.93 m)에서 좌표를 역산해 메시 위치를 `-3.15, 0, -0.53`, 크기 배율 `1.0`으로 맞췄습니다.
